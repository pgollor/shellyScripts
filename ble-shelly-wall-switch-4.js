 ...
 * button push
 * 
 * @2024 Modified by Pascal Gollor
 * most of the code comes from https://github.com/ALLTERCO/shelly-script-examples/blob/main/ble-shelly-btn.js
 */
...
{
  // check if button is a multiple button and if one of the buttons were pressed
  if ("Buttons" in BTHparsed)
  {
    let idx = null;
    let oneUEnull = false;
    for (idx in BTHparsed.Button)
    {
      if (BTHparsed.Button[idx] > 0 && BTHparsed.Button[idx] <= 0x80)
      {
        oneUEnull = true;
        break;
      }
    ...
    // return of no button as event type unequal None
    if (oneUEnull == false)
    {
      return;
    }
  ...
  {
    BTHparsed["Button"] = {"0": BTHparsed["Button"]};
    BTHparsed["Buttons"] = 1;
  }

  // Emits event
  let btn = 0;
  while (btn < BTHparsed["Buttons"])
  {
    if (BTHparsed["Button"][btn] > 0)
    {
      Shelly.emitEvent("BLU_BUTTON", {
        addr: BTHparsed.addr,
        rssi: BTHparsed.rssi,
        Button: btn,
        Type: BTHparsed["Button"][btn],
        Battery: BTHparsed.Battery,
      });
    
      // Toggles the relay on the device
      if (BTHparsed["Button"][btn] == 1)
      {
        Shelly.call("Switch.Toggle", { id: btn });
      }
    }
...
let BTHomeDecoder = {
  ...
  unpack: function (buffer) {
    ...
    // removing BTH header like verison and so on
    ...
    {
      // try to get data meta information from BTH struct
      ...
      // reduce buffer by one to remove Object ID
      buffer = buffer.slice(1);
 
      // read object value from buffer
      _value = this.getBufValue(_bth.t, buffer);
      if (_value === null) break;
      
      // multiply data if data format is float
      if (typeof _bth.f !== "undefined") _value = _value * _bth.f;
      
      // store value in result array
      // check for multiple sensors, events or so
      if (_bth.n in result)
      {
        // get object index
        let idx = 2;
        if (_bth.n + "s" in result)
        {
          idx = result[_bth.n + "s"] + 1;
        }
        else
        {
          // convert value into array
          let old = result[_bth.n];
          result[_bth.n] = {0: old};
        }
        // store button index
        result[_bth.n + "s"] = idx;
      ...
      // remove object value from buffer
